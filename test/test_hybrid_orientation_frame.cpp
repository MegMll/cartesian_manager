...
TEST(HybridOrientationFrame, UsesOrthonormalFallbackAtBothVerticalPoles)
{
  ...
  const Eigen::Matrix3d upward = hybrid_frame.update(
      Eigen::Quaterniond::Identity(), Eigen::Vector3d::Zero(), kConeAngle, kReleaseThreshold, hybrid_state);
  EXPECT_TRUE(upward.isApprox(Eigen::Matrix3d::Identity(), kTolerance));

  hybrid_frame.reset(hybrid_state);
...
TEST(HybridOrientationFrame, PreservesAxisContinuityUntilRelease)
{
  ...
  const Eigen::Matrix3d before = hybrid_frame.update(
      orientationFromToolZ(before_z), active_input, kConeAngle, kReleaseThreshold, hybrid_state);
  const Eigen::Matrix3d inside = hybrid_frame.update(
      orientationFromToolZ(inside_z), active_input, kConeAngle, kReleaseThreshold, hybrid_state);
  const Eigen::Matrix3d after = hybrid_frame.update(
      orientationFromToolZ(after_z), active_input, kConeAngle, kReleaseThreshold, hybrid_state);
  ...
  const Eigen::Matrix3d released = hybrid_frame.update(
      orientationFromToolZ(after_z), Eigen::Vector3d::Zero(), kConeAngle, kReleaseThreshold, hybrid_state);
...
TEST(HybridOrientationFrame, MapsAllOrientationFramesToBase)
{
  ...
  const Eigen::Vector3d base = manager_core::mapAngularInputToBase(
      "base_frame", quarter_turn, input, kConeAngle, kReleaseThreshold, hybrid_state,
      &base_frame);
  const Eigen::Vector3d effector = manager_core::mapAngularInputToBase(
      "effector_frame", quarter_turn, input, kConeAngle, kReleaseThreshold, hybrid_state,
      &effector_frame);
...
TEST(HybridOrientationFrame, ResetDropsContinuityState)
{
  ...
  hybrid_frame.update(orientationFromToolZ(first_z), Eigen::Vector3d::UnitX(), kConeAngle, 0.0, hybrid_state);
  const Eigen::Matrix3d continuous = hybrid_frame.update(
      orientationFromToolZ(second_z), Eigen::Vector3d::UnitX(), kConeAngle, 0.0, hybrid_state);
  hybrid_frame.reset(hybrid_state);
